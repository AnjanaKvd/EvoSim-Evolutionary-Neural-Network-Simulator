...
def _sim_worker(cfg: dict, stop_evt: threading.Event, out_q: queue.Queue):
    ...
    # Speed map: 1=Slow, 5=Max (no delay)
    speed_map = {1: 0.5, 2: 0.2, 3: 0.1, 4: 0.05, 5: 0.0}
    sim.delay = speed_map.get(cfg["speed"], 0.0)
    ...
    try:
        while not stop_evt.is_set():
            ...
            if sim.generation == 0:
                genomes = [
                    random_genome(cfg["genome_size"], sim.rng)
    ...
    def _run_hooked(self, initial_genomes, stop_evt):
        ...
        for gen_idx in range(self.max_generations):
            ...
            if self.on_gen_callback:
                self.on_gen_callback(
                    gen_idx, stats, self.world, self.current_gen_creatures, survivors
                )
            if not survivors:
                break
